...
describe("WordsService", () => {
  ...
  describe("findAllByUser", () => {
    it("should return words for a user", async () => {
      ...
      await testDb.createWord({ term: "word2", userId: testUser.id, isPublic: false });
  ...
  describe("remove", () => {
    ...
    it("should throw ForbiddenException when non-owner tries to delete", async () => {
      ...
      const word = await testDb.createWord({ term: "not-mine", userId: owner.id, isPublic: false });
  ...
  describe("search", () => {
    ...
    it("should search for public words", async () => {
      await testDb.createWord({ term: "searchable", userId: testUser.id, isPublic: true });
      await testDb.createWord({ term: "another-search", userId: testUser.id, isPublic: true });
      const paginationDto: PaginationDto = { page: 1, limit: 10, offset: 0 };
    ...
    it("should include user's private words in search results when userId provided", async () => {
      await testDb.createWord({ term: "my-secret", userId: testUser.id, isPublic: false });
      const paginationDto: PaginationDto = { page: 1, limit: 10, offset: 0 };
