  ...
  describe("findFeeds", () => {
    it("should generate correct query with user IDs filter", () => {
      ...
      expect(listQuery.toQuery()).toBe(
        'select "definitions"."id" as "id", "definitions"."content" as "content", "definitions"."word_id" as "wordId", "definitions"."user_id" as "userId", "definitions"."created_at" as "createdAt", "definitions"."updated_at" as "updatedAt", "users"."nickname" as "nickname", "users"."profile_picture" as "profilePicture", "definitions"."tags" as "tags", "words"."term" as "term", "terms"."number" as "termNumber" from "vw_definitions_with_likes" as "definitions" left join "users" on "definitions"."user_id" = "users"."id" left join "words" on "definitions"."word_id" = "words"."id" left join "terms" on "words"."term" = "terms"."text" where "definitions"."deleted_at" is null and "definitions"."user_id" in (\'user-1\', \'user-2\') and "words"."deleted_at" is null and "definitions"."is_public" = true order by "definitions"."created_at" desc limit 10',
      );
    });
  ...
  describe("findAllFeeds", () => {
    it("all feeds with cursor", () => {
      ...
      expect(feed.toQuery()).toBe(
        'select "definitions"."id" as "id", "definitions"."content" as "content", "definitions"."word_id" as "wordId", "definitions"."user_id" as "userId", "definitions"."created_at" as "createdAt", "definitions"."updated_at" as "updatedAt", "users"."nickname" as "nickname", "users"."profile_picture" as "profilePicture", "words"."term" as "term", "definitions"."tags" as "tags", "terms"."number" as "termNumber" from "vw_definitions_with_likes" as "definitions" left join "users" on "definitions"."user_id" = "users"."id" left join "words" on "definitions"."word_id" = "words"."id" left join "terms" on "words"."term" = "terms"."text" where "definitions"."deleted_at" is null and "words"."deleted_at" is null and "definitions"."is_public" = true and "definitions"."created_at" < \'2026-01-27T12:48:19\' order by "definitions"."created_at" desc limit 15',
      );
    });
  });
